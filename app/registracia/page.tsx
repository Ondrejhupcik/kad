    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 flex flex-col items-center">
          ...
          <CardTitle className="text-2xl font-bold text-center">Registrácia</CardTitle>
          <CardDescription className="text-center">
            Vytvorte si nový účet pre váš kaderníctvo
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleRegister}>
          ...
          <CardFooter className="flex flex-col space-y-4">
            <Button type="submit" className="w-full" disabled={loading}>
              {loading ? 'Registrácia prebieha...' : 'Zaregistrovať sa'}
            </Button>
            <p className="text-sm text-center text-slate-600">
              Už máte účet?{' '}
              <Link href="/prihlasenie" className="text-slate-900 font-medium hover:underline">
                Prihláste sa
              </Link>
            </p>
          </CardFooter>
